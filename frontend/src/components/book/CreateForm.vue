<template>
    <div class="mt-5">
        <form @submit.prevent="create" class="w-[600px] mx-auto">
            <h3 class="text-2xl font-bold text-center mb-4">Create a book</h3>
            <div class="space-y-4">
                <div>
...
const { contentOwners, publishers, errors } = storeToRefs(store);
const { getContentOwners, getPublishers, createBook } = store;
...
const create = async() => {
    ...
    console.log(form)
    let res = await createBook(form);

    //remove spinner
    span.value.textContent = 'Create';
